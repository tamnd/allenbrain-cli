...
describes what users get. Pick a format with `-o`, or let allenbrain choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
allenbrain <command> -o table   # aligned columns for reading
allenbrain <command> -o jsonl   # one JSON object per line, for piping
allenbrain <command> -o json    # a single JSON array
allenbrain <command> -o csv     # spreadsheet friendly
allenbrain <command> -o tsv     # tab-separated
allenbrain <command> -o url     # just the URL column
allenbrain <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
allenbrain <command> --fields id,title,url
```
...
```bash
allenbrain <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
allenbrain <command>            # a table, because this is a terminal
allenbrain <command> | wc -l    # JSONL, because this is a pipe
```
